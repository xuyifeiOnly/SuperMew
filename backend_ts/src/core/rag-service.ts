...
const DEFAULT_BIGMODEL_RERANK_ENDPOINT =
  "https://open.bigmodel.cn/api/paas/v4/rerank";

let rerankCooldownUntilMs = 0;
...
const formatRerankError = (error: unknown): string => {
  ...
  if (raw === "rerank_timeout") {
    return "Rerank 请求超时，已自动回退到原始召回顺序";
  }
  if (raw === "rerank_cooldown") {
    return "Rerank 触发限流，已进入冷却期，已自动回退到原始召回顺序";
  }
  if (
  ...
  if (raw.startsWith("HTTP 429")) {
    return `Rerank 服务触发限流（429），已自动回退到原始召回顺序；${raw}`;
  }
...
export class RagService {
  ...
  private async gradeDocuments(
    question: string,
    context: string,
  ): Promise<{ score: string; route: string; error?: string }> {
    ...
    try {
      ...
      const response = await graderModel
        .withStructuredOutput(GradeSchema)
        .invoke(prompt);
      const score =
        normalizeText(response.binary_score || "").toLowerCase() === "yes"
          ? "yes"
          : context
            ? "yes"
            : "no";
  ...
  private async chooseRewriteStrategy(
    question: string,
  ): Promise<"step_back" | "hyde" | "complex"> {
  ...
  private async rerankDocuments(
    ...
    const docsWithRank = docs.map((doc, index) => ({
      ...doc,
      rrf_rank: index + 1,
    }));
    ...
    if (Date.now() < rerankCooldownUntilMs) {
      meta.rerank_error = formatRerankError("rerank_cooldown");
      return { docs: docsWithRank.slice(0, topK), meta };
    }
    ...
    try {
      ...
      const timeoutMs = Math.max(
        1000,
        Number(env.rerankTimeoutMs || 0) || 8000,
      );
      ...
      if (!response.ok) {
        if (response.status === 429) {
          const cooldownMs =
            Math.max(1, Number(env.rerankCooldownSeconds || 0) || 60) * 1000;
          rerankCooldownUntilMs = Date.now() + cooldownMs;
        }
      ...
      return {
        docs: (reranked.length ? reranked : docsWithRank).slice(0, topK),
        meta,
      };
    } catch (error) {
      ...
      meta.rerank_error = formatRerankError(
        isAbort ? "rerank_timeout" : message,
      );
      return { docs: docsWithRank.slice(0, topK), meta };
    }
  }
  ...
  private async autoMergeDocuments(
    ...
    const mergeOnce = async (
    ...
    ): Promise<{ docs: RetrievedChunk[]; replaced: number }> => {
      ...
      const parentDocs =
        await this.parentChunkStore.getDocumentsByIds(parentIds);
      const parentMap = new Map(
        parentDocs.map((item) => [item.chunk_id, item]),
      );
      ...
      for (const doc of merged) {
        const key =
          doc.chunk_id || `${doc.filename}:${doc.page_number}:${doc.text}`;
        if (seen.has(key)) {
          continue;
        }
        seen.add(key);
    ...
    return {
      ...
      meta: {
        ...
        auto_merge_steps:
          Number(first.replaced > 0) + Number(second.replaced > 0),
      },
    };
  }

  private async keywordFallbackRetrieve(
    query: string,
    topK: number,
    filterExpr: string,
  ...
  async retrieveDocuments(
    ...
    const candidateK = Math.max(topK * 2, topK);
    const effectiveRoles = userRoles.length ? userRoles : ["user"];
    const accessFilter = await buildAccessibleFilterExpr(
      effectiveRoles,
      `chunk_level == ${env.leafRetrieveLevel}`,
    );
    ...
    try {
      ...
      try {
        ...
        const retrieved = await this.milvusManager.denseRetrieve(
          denseEmbedding,
          candidateK,
          filterExpr,
        );
        if (!retrieved.length) {
          return {
            ...
            meta: {
              ...
              rerank_endpoint: getRerankEndpoint() || null,
              rerank_error: null,
              retrieval_mode: "dense_fallback_empty",
              candidate_k: candidateK,
              leaf_retrieve_level: env.leafRetrieveLevel,
              current_role: effectiveRoles[0],
              current_roles: effectiveRoles,
  ...
  async searchKnowledgeBase(
    ...
    userRoles: Role[] = ["user"],
    emit?: EmitStep,
  ): Promise<{ text: string; ragTrace: RagTrace }> {
    ...
    const effectiveRoles = userRoles.length ? userRoles : ["user"];
    const initial = await this.retrieveDocuments(query, 2, effectiveRoles);
    ...
    let ragTrace: RagTrace = {
      ...
      current_roles:
        (initial.meta.current_roles as string[] | undefined) ?? effectiveRoles,
      ...
      auto_merge_threshold: Number(
        initial.meta.auto_merge_threshold ?? env.autoMergeThreshold,
      ),
      auto_merge_replaced_chunks: Number(
        initial.meta.auto_merge_replaced_chunks ?? 0,
      ),
    ...
    const collect = async (
      ...
      const response = await this.retrieveDocuments(
        sourceQuery,
        5,
        effectiveRoles,
      );
      ...
      retrievalMode =
        retrievalMode || String(response.meta.retrieval_mode ?? "");
      candidateK = candidateK || Number(response.meta.candidate_k ?? 0);
      leafRetrieveLevel = Number(
        response.meta.leaf_retrieve_level ?? leafRetrieveLevel,
      );
      autoMergeEnabled = Boolean(response.meta.auto_merge_enabled);
      autoMergeApplied =
        autoMergeApplied || Boolean(response.meta.auto_merge_applied);
      autoMergeThreshold = Number(
        response.meta.auto_merge_threshold ?? autoMergeThreshold,
      );
      autoMergeReplacedChunks += Number(
        response.meta.auto_merge_replaced_chunks ?? 0,
      );
      ...
      rerankEndpoint =
        rerankEndpoint || String(response.meta.rerank_endpoint ?? "");
      currentRole = String(response.meta.current_role ?? currentRole);
      currentRoles =
        (response.meta.current_roles as string[] | undefined) ?? currentRoles;
      accessScope = String(response.meta.access_scope ?? accessScope);
      restrictedFileCount = Math.max(
        restrictedFileCount,
        Number(response.meta.restricted_file_count ?? 0),
      );
